...
// grabs the current user with the backend route based on if a token is in localstorage.
  componentDidMount = () => {
    ...
    token
      ? fetch("https://localhost:3000/api/v1/current_user", {
          method: "GET",
          headers: {
...
// login handler sends over userInfo body
  ...
  handleExperienceCard = (e, businessInfo) => {
    ...
    if(e.target.className === 'fab3' || e.target.className === 'fa fa-thumbs-up fa-3x') {
      ...
      fetch(`https://localhost:3000/api/v1/experiences/${businessInfo.id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Accepts': 'application/json'
  ...
  handleConfirmActivity = (activityInfo, mins) => {
    fetch('https://localhost:3000/api/v1/activities', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accepts': 'application/json'
      ...
      .then(activity => {
        fetch('https://localhost:3000/api/v1/experiences', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Accepts': 'application/json'
